...
std::function<void(Core8051* pt)> Core8051::InstructionDecoder[] =
{
   ...
   { [](Core8051* pt){pt->PC++; if(pt->GetBit(pt->code[pt->PC++])
					{
					pt->ClearBit(pt->code[pt->PC - Byte_t{1}];
					pt->PC += pt->code[pt->PC++].signedByte;
					}
				else
					{pt->PC++;}} }, //JBC bt addr, code addr 10
   { [](Core8051* pt){pt->PC++;} }, //ACALL code addr 11
   { [](Core8051* pt){pt->PC++; Word_t temp;
				temp.highByte = pt->code[pt->PC++];
				temp.lowByte = pt->code[pt->PC++];
				pt->SP++; pt->ram[pt->SP] = pt->PC.lowByte;
				pt->SP++; pt->ram[pt->SP] = pt->PC.highByte;} }, //LCALL code addr 12
   { [](Core8051* pt){pt->PC++; bool temp = pt->PSW.b7;
				pt->PSW.b7 = pt->ACC.b0;
				pt->ACC.byte >>= 1;
				pt->ACC.b7 = temp;} }, //RRC A 13
   ...
   { [](Core8051* pt){pt->PC++; Word_t temp{0x100};
				temp.lowByte = pt->code[pt->PC++];
				pt->PC = temp;} }, //AJMP code addr 21
   { [](Core8051* pt){pt->PC++; pt->PC.highByte = pt->ram[pt->SP--];
				pt->PC.lowByte = pt->ram[pt->SP--];} }, //RET 22
   { [](Core8051* pt){pt->PC++; bool temp = pt->ACC.b7;
				pt->ACC.byte <<= 1;
				pt->ACC.b0 = temp;} }, //RL A 23
   ...
   { [](Core8051* pt){pt->PC++; pt->PC.highByte = pt->ram[pt->SP--];
				pt->PC.lowByte = pt->ram[pt->SP--];} }, //RETI 32
   { [](Core8051* pt){pt->PC++; bool temp = pt->PSW.b7;
				pt->PSW.b7 = pt->ACC.b7;
				pt->ACC.byte <<= 1;
				pt->ACC.b0 = temp;} }, //RLC A 33
   ...
   { [](Core8051* pt){pt->PC++; if(PSW.b7)
					{pt->PC += pt->code[pt->PC++].signedByte}
				else
					{pt->PC++;}} }, //JC code addr 40
   { [](Core8051* pt){pt->PC++; Word_t temp{0x200};
				temp.lowByte = pt->code[pt->PC++];
				pt->PC = temp;} }, //AJMP code addr 41
   { [](Core8051* pt){pt->PC++; pt->ram[pt->code[pt->PC]] &= pt->ACC; pt->PC++;} }, //ORL data addr,A 42
   { [](Core8051* pt){pt->PC++; pt->ram[pt->code[pt->PC++]] &= pt->code[pt->PC++];} }, //ORL data addr,#data 43
   ...
   { [](Core8051* pt){pt->PC++; if(pt->ACC)
					...
					{pt->PC++;};} }, //JZ code addr	60
   { [](Core8051* pt){pt->PC++; Word_t temp{0x300};
				temp.lowByte = pt->code[pt->PC++];
				pt->PC = temp;} }, //AJMP codeaddr 61
   { [](Core8051* pt){pt->PC++; pt->ram[pt->code[pt->PC]] ^= pt->ACC; pt->PC++;} }, //XRL data addr,A 62
   { [](Core8051* pt){pt->PC++; pt->ram[pt->code[pt->PC++]] ^= pt->code[pt->PC++}];} }, //XRL data addr,#data 63
   ...
   { [](Core8051* pt){pt->PC++; pt->ram[pt->code[pt->PC]] = pt->code[pt->PC + Byte_t{1}];
				pt->PC += Word_t{2};} }, //MOV data addr,#data 75
   ...
   { [](Core8051* pt){pt->PC++; pt->ram[pt->code[pt->PC++]] = pt->ram[pt->code[pt->PC++]];} }, //MOV data addr,data addr 85
   { [](Core8051* pt){pt->PC++; pt->ram[pt->code[pt->PC++]] = pt->ram[pt->Register(0)];} }, //MOV data addr,@R0 86
   { [](Core8051* pt){pt->PC++; pt->ram[pt->code[pt->PC++]] = pt->ram[pt->Register(1)];} }, //MOV data addr,@R1	87
   ...
   { [](Core8051* pt){pt->PC++; pt->ClearBit(PSW_7);} }, //CLR C3
   { [](Core8051* pt){pt->PC++; Byte_t temp; 
				temp.n0 = pt->ACC.n1;
				temp.n1 = pt->ACC.n0;
				pt->ACC.byte = temp.byte;} }, //SWAP A C4
